...
const Tabs = ({ goals, setGoals, user }: TabsProps) => {
  ...
  const filterByCategories = sections.filter(
    (section) => section.name !== "Prio"
  )
  const filterPrioPD = sections[0].goals.filter(
    (goal) => goal.category === "personalDevelopment"
  )
  const filterPrioCI = sections[0].goals.filter(
    (goal) => goal.category === "customerInteraction"
  )
  const filterPrioBG = sections[0].goals.filter(
  ...
  return (
    <div className="tabs-wrapper">
      <Modal goals={goals} setGoals={setGoals} setShow={setShow} show={show} />
